...
export default function ActiveChat({ activeChat }) {
  ...
  return (
    <>
      ...
      <Box
        sx={{ borderRadius: "10px", height: "100%", p: 3 }}
        className="activeChatBackground"
      >
        {data.oneMatch.messages.map((message) => (
          <>
            <Box
              sx={{ display: "flex", width: 1, py: 1 }}
              className={message.user._id == selfId ? "myText" : "theirText"}
            >
              <Box
                sx={{
                  width: "auto",
                ...
                className={
                  message.user._id == selfId ? "myBubble" : "theirBubble"
                }
              >
                {" "}
                {message.messageText}
      ...
      >
        ...
        <Button
          variant="contained"
          sx={{ width: "150px", height: "100%", maxHeight: "100px" }}
          onClick={handleSend}
        >
          Send
        </Button>
